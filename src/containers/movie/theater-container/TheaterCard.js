...
class TheaterCard extends Component {
  ...
  getScreenObject = (theater) => {
    ...
    getScreens(selectedMovie.movieId, theater.theaterId).then((screens) =>
      this.setState({
        screens: screens,
      })
    );
  ...
  handleSelectedShowTime = (theater, time) => {
    ...
    const screen = screens.filter((screen) =>
      screen.screenTimes
        .map((screenTime) => screenTime.showTiming)
        .includes(time)
    );
    const screenTime = screen[0].screenTimes.filter(
      (screenTime) => screenTime.showTiming === time
    );
    const seatPlanId = screenTime[0].showDateList.filter(
      (showDate) => showDate.moviePlayingDate.split("T")[0] === selectedDate
    )[0].seatPlanId;
